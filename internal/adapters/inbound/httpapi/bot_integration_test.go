...
func TestCreateTurnForBot_EnqueuesTurnOnExistingSession(t *testing.T) {
	...
	first, err := CreateTurnForBot(ctx, pool, sessions, turns, plans, nil, auditLog, registry, created.ID, "first mention", nil, false, false, pgtype.UUID{}, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("CreateTurnForBot (first): %v", err)
	}

	second, err := CreateTurnForBot(ctx, pool, sessions, turns, plans, nil, auditLog, registry, created.ID, "second concurrent mention", nil, false, false, pgtype.UUID{}, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
...
func TestCreateTurnForBot_WritesAuditLogRowWithActor(t *testing.T) {
	...
	turnRow, err := CreateTurnForBot(ctx, pool, sessions, turns, plans, nil, auditLog, registry, created.ID, "please take a look", nil, false, false, actor.ID, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
...
func TestCreateTurnForBot_PlanAwaitingApproval_PreservesSentinel(t *testing.T) {
	...
	_, err = CreateTurnForBot(ctx, pool, sessions, turns, plans, nil, auditLog, registry, created.ID, "please build this now", nil, false, false, pgtype.UUID{}, nil, nil, nil, nil, nil, nil, nil, nil)
	if err == nil {
