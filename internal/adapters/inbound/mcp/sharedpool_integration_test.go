...
// This package's integration suite (integration_test.go, package
// mcp_test -- no white-box split needed here) proves the MCP surface's
// own parity table against a REAL Postgres-backed rig, mirroring
// internal/adapters/inbound/httpapi's own *_integration_test.go
// convention exactly.
//
...
// TriggerDispatch/EnsureDispatched at all -- every 180 tool is a plain
// read through the bridge (bridge.go's own doc comment), never a
// session-actor dispatch -- so condition #1 from httpapi's own doc
// comment (pool-before-registry ordering, so registry.Shutdown() always
// drains before a truncate runs) simply does not apply here. This
// package also never calls t.Parallel() in integration_test.go itself
// (this package's own non-integration-tagged unit tests DO use real
// httptest.Request/Recorder round trips, but none of them touch
// pgxpool/testcontainers/the DB at all -- verified -- so they pose no
// risk to the shared container's truncate/restore cycle).
//
...
func TestMain(m *testing.M) {
	...
	if err != nil {
		log.Fatalf("mcp: start shared integration-test container: %v", err)
	}

	if err := runMigrations(connStr); err != nil {
		log.Fatalf("mcp: run migrations against shared integration-test container: %v", err)
	}

	pool, err := narvipg.NewPool(ctx, connStr)
	if err != nil {
		log.Fatalf("mcp: open shared integration-test pool: %v", err)
	}

	if err := prepareDatabaseReset(ctx, pool); err != nil {
		log.Fatalf("mcp: prepare shared integration-test database reset: %v", err)
	}
	...
	if err := testcontainers.TerminateContainer(container); err != nil {
		log.Printf("mcp: terminate shared integration-test container: %v", err)
	}
	os.Exit(code)
}
...
func IntegrationTestPoolAndConnStr(t *testing.T) (*pgxpool.Pool, string) {
	t.Helper()
	if sharedPool == nil {
		t.Fatal("mcp: IntegrationTestPoolAndConnStr called before TestMain finished setting up the shared pool")
	}
...
func resetSharedTestDatabase(t *testing.T) {
	...
	if _, err := sharedPool.Exec(ctx, truncateStatement); err != nil {
		t.Fatalf("mcp: reset shared integration-test database (truncate): %v", err)
	}
	for _, stmt := range restoreStatements {
		if _, err := sharedPool.Exec(ctx, stmt); err != nil {
			t.Fatalf("mcp: reset shared integration-test database (restore seed data): %v", err)
		}
	}
}
