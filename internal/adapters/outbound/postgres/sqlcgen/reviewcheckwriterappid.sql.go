...
// versions:
//   sqlc v1.31.1
// source: reviewcheckwriterappid.sql
...
const getReviewCheckWriterAppID = `-- name: GetReviewCheckWriterAppID :one
...
func (q *Queries) GetReviewCheckWriterAppID(ctx context.Context) (int64, error) {
	row := q.db.QueryRow(ctx, getReviewCheckWriterAppID)
	var app_id int64
	err := row.Scan(&app_id)
	return app_id, err
}

const upsertReviewCheckWriterAppID = `-- name: UpsertReviewCheckWriterAppID :exec
...
func (q *Queries) UpsertReviewCheckWriterAppID(ctx context.Context, appID int64) error {
	_, err := q.db.Exec(ctx, upsertReviewCheckWriterAppID, appID)
	return err
}
