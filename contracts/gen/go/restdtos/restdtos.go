...
type AcceptReviewVerdictRequest struct {
	...
	Justification string `json:"justification" yaml:"justification" mapstructure:"justification"`
...
func (j *AcceptReviewVerdictRequest) UnmarshalJSON(value []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(value, &raw); err != nil {
		return err
	}
	if _, ok := raw["justification"]; raw != nil && !ok {
		return fmt.Errorf("field justification in AcceptReviewVerdictRequest: required")
	}
	if _, ok := raw["prNumber"]; raw != nil && !ok {
		return fmt.Errorf("field prNumber in AcceptReviewVerdictRequest: required")
	}
	...
	*j = AcceptReviewVerdictRequest(plain)
	return nil
}
...
// an MCP client. The server generates the client_id. Every redirect URI must be at
// most 2048 bytes of printable ASCII: https with any host, or http on 127.0.0.1,
// [::1] or localhost, with a valid port if it names one, no fragment and no
...
type CreateMCPClientRequest struct {
	...
	// Optional https homepage of the client: at most 2048 bytes of printable ASCII,
	// with a host and no userinfo, its host written in plain ASCII like a redirect
	// URI's -- anything else is refused 400. The consent page shows its host.
...
type DecisionInboxItem struct {
	...
	// (internal/domain/reviewverdict.Acceptance.Applicable) -- null for a PR that was
	// never accepted, whose acceptance was since revoked, or whose acceptance no
	// longer binds to the current verdict (a new attempt, a moved base, a changed
	// ancestor chain -- §21.1b's own three invalidating triggers). Display only: this
	// never changes kind -- an accepted PR still renders needs_review, never
...
// the twin handler in-process (technical plan §43.7's bridge).
type GetSessionToolRequest struct {
	// The session id, matching Session.id's own format exactly. "format":"uuid" is
	// enforced by this package's own bridge
	// (internal/adapters/inbound/mcp/schemas.go's validateArguments compiles this
	// schema with format assertions on), so a malformed value never reaches the twin
	// at all -- reported as a tool execution error (isError:true), not a JSON-RPC
	// protocol code.
...
type Integration struct {
	...
	// platform.Config AND this deployment has actually enabled that surface
	// (platform.Config.IngressEnabled, NARVI_INGRESS_ENABLED, §12.5's own
	// ingress-optionality decision) -- NEVER the secrets themselves, not even shaped:
	// no prefix, no length, no masked form, nothing about WHAT configures it. A
	// surface this deployment never enabled reads false; a surface missing even one
	// required value while enabled also reads false, never a half-working "connected
	// but broken" state -- see internal/domain/integrations's own
	// ConfiguredSlack/ConfiguredLinear/ConfiguredGitHub doc comments for the exact
	// set each surface checks. A genuine two-valued field: platform.Load's own
	// fail-fast boot gate refuses to start an enabled-but-incomplete surface, so a
	// running deployment can only ever be observed in one of two states per surface
	// -- enabled and fully configured (true), or not enabled here (false) -- never a
	// third 'enabled but broken' state this field would need to distinguish. A screen
	// must NOT present this as a live health check of anything.
...
// The narvi_list_sessions MCP tool's own input (technical plan §43.7) -- the tool
...
// filter/limit carry the REAL value-space constraints below, validated by this
// package's own bridge (internal/adapters/inbound/mcp/schemas.go's
// validateArguments) before either BuildRequest or the twin ever sees an argument
// -- a prior revision of this $def deliberately omitted "enum"/"minimum" on the
// mistaken premise that the pinned MCP SDK's own raw Server.AddTool path enforced
// them; it enforces nothing on that path, so the omission bought no parity
// benefit, only a weaker contract.
type ListSessionsToolRequest struct {
	// Matches GET /api/sessions's own ?filter= values exactly
	// (httpapi/listsessions.go) -- "mine" (the default when omitted) or "all". A
	// value outside this enum fails validation before the twin is ever invoked, and
	// is reported as a tool execution error (isError:true), never a JSON-RPC protocol
	// code -- see doc.go's own HTTP-outcome-to-MCP-outcome table.
	Filter *ListSessionsToolRequestFilter `json:"filter,omitempty,omitzero" yaml:"filter,omitempty" mapstructure:"filter,omitempty"`

	// Matches GET /api/sessions's own ?limit= exactly
	// (httpapi.listSessionsDefaultLimit/listSessionsMaxLimit): omitted means the
	// route's own default. minimum matches the REST route's own rejection (<= 0 is
	// refused, "malformed limit"). Deliberately NO "maximum" here, for two
	// independent reasons: tools/contractscompat's own closed keyword allowlist does
	// not recognize "maximum" yet (it fails closed on any schema node using it -- see
	// that tool's own keywords.go), and a value above listSessionsMaxLimit is not
	// actually a REST-route REJECTION either way -- like the REST route itself, it is
	// silently clamped server-side, so declaring a hard "maximum" here would make
	// this tool's own contract reject a value the REST route accepts, a real behavior
	// divergence "restore the real constraints" is not asking for.
	Limit *int `json:"limit,omitempty,omitzero" yaml:"limit,omitempty" mapstructure:"limit,omitempty"`
}

type ListSessionsToolRequestFilter string

const ListSessionsToolRequestFilterAll ListSessionsToolRequestFilter = "all"
const ListSessionsToolRequestFilterMine ListSessionsToolRequestFilter = "mine"

var enumValues_ListSessionsToolRequestFilter = []interface{}{
	"mine",
	"all",
}

// UnmarshalJSON implements json.Unmarshaler.
func (j *ListSessionsToolRequestFilter) UnmarshalJSON(value []byte) error {
	var v string
	if err := json.Unmarshal(value, &v); err != nil {
		return err
	}
	var ok bool
	for _, expected := range enumValues_ListSessionsToolRequestFilter {
		if reflect.DeepEqual(v, expected) {
			ok = true
			break
		}
	...
	if !ok {
		return fmt.Errorf("invalid value (expected one of %#v): %#v", enumValues_ListSessionsToolRequestFilter, v)
	}
	*j = ListSessionsToolRequestFilter(v)
	return nil
}

// UnmarshalJSON implements json.Unmarshaler.
func (j *ListSessionsToolRequest) UnmarshalJSON(value []byte) error {
	type Plain ListSessionsToolRequest
	var plain Plain
	if err := json.Unmarshal(value, &plain); err != nil {
		return err
	}
	...
	*j = ListSessionsToolRequest(plain)
	return nil
}
...
type MCPAuthorization struct {
	...
	// (migrations/000141_mcp_oauth.up.sql). How the client came to be registered
	// (technical plan §43.15): preregistered by an administrator; metadata_document,
	// identified by an https URL whose client ID metadata document the server fetched
	// (clientId is that URL, and its host is the client's identity); or dynamic,
	// registered by the client itself through RFC 7591 (off by default). An OPEN enum
	// (manifest.json's openEnums): a consumer MUST tolerate a value it does not
	// recognise.
	ClientKind MCPAuthorizationClientKind `json:"clientKind" yaml:"clientKind" mapstructure:"clientKind"`

	// The client's display name, as registered -- chosen by the client itself unless
	// clientKind is preregistered, so never an identity on its own (technical plan
	// §43.15).
	ClientName string `json:"clientName" yaml:"clientName" mapstructure:"clientName"`
...
// One review_verdict_acceptances row's own REST wire shape ('human acceptance of a
// verdict the engine refuses', §21.1b,
// migrations/000135_review_verdict_acceptances.up.sql) -- returned by
// accept-verdict and revoke-verdict-acceptance so a caller can confirm the
// resulting state.
type ReviewVerdictAcceptance struct {
	// AcceptedAt corresponds to the JSON schema field "acceptedAt".
	AcceptedAt time.Time `json:"acceptedAt" yaml:"acceptedAt" mapstructure:"acceptedAt"`

	// The accepting maintainer+'s own user id.
	AcceptedBy string `json:"acceptedBy" yaml:"acceptedBy" mapstructure:"acceptedBy"`

	// The accepted verdict's own turns.id, carried verbatim for display/audit -- null
	// for a pre-§21.1-amendment verdict.
	AttemptId ReviewVerdictAcceptanceAttemptId `json:"attemptId" yaml:"attemptId" mapstructure:"attemptId"`
	...
	RevokedBy ReviewVerdictAcceptanceRevokedBy `json:"revokedBy" yaml:"revokedBy" mapstructure:"revokedBy"`

	// review_verdicts.id -- the ONE verdict this acceptance binds to
	// (internal/domain/reviewverdict.Acceptance.Applicable's own doc comment).
	VerdictId string `json:"verdictId" yaml:"verdictId" mapstructure:"verdictId"`
}
...
// UnmarshalJSON implements json.Unmarshaler.
func (j *ReviewVerdictAcceptance) UnmarshalJSON(value []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(value, &raw); err != nil {
		return err
	}
	if _, ok := raw["acceptedAt"]; raw != nil && !ok {
		return fmt.Errorf("field acceptedAt in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["acceptedBy"]; raw != nil && !ok {
		return fmt.Errorf("field acceptedBy in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["attemptId"]; raw != nil && !ok {
		return fmt.Errorf("field attemptId in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["headSha"]; raw != nil && !ok {
		return fmt.Errorf("field headSha in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["id"]; raw != nil && !ok {
		return fmt.Errorf("field id in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["justification"]; raw != nil && !ok {
		return fmt.Errorf("field justification in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["prNumber"]; raw != nil && !ok {
		return fmt.Errorf("field prNumber in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["reason"]; raw != nil && !ok {
		return fmt.Errorf("field reason in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["repoFullName"]; raw != nil && !ok {
	...
	if _, ok := raw["revokedBy"]; raw != nil && !ok {
		return fmt.Errorf("field revokedBy in ReviewVerdictAcceptance: required")
	}
	if _, ok := raw["verdictId"]; raw != nil && !ok {
		return fmt.Errorf("field verdictId in ReviewVerdictAcceptance: required")
	}
	type Plain ReviewVerdictAcceptance
	var plain Plain
	if err := json.Unmarshal(value, &plain); err != nil {
		return err
	}
	*j = ReviewVerdictAcceptance(plain)
	return nil
}
...
// POST /api/decision-inbox/revoke-verdict-acceptance's own request body -- the
// SAME maintainer+ role as accept-verdict (§21.1b, never a stricter one), scoped
// to repoFullName exactly like RetireFalsePositivePattern's own identical
// audit-fix precedent.
type RevokeReviewVerdictAcceptanceRequest struct {
	// The acceptance's own id (ReviewVerdictAcceptance.id) -- never the verdict id or
	// the PR number.
	Id string `json:"id" yaml:"id" mapstructure:"id"`
...
// UnmarshalJSON implements json.Unmarshaler.
func (j *RevokeReviewVerdictAcceptanceRequest) UnmarshalJSON(value []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(value, &raw); err != nil {
		return err
	}
	if _, ok := raw["id"]; raw != nil && !ok {
		return fmt.Errorf("field id in RevokeReviewVerdictAcceptanceRequest: required")
	}
	if _, ok := raw["repoFullName"]; raw != nil && !ok {
		return fmt.Errorf("field repoFullName in RevokeReviewVerdictAcceptanceRequest: required")
	}
	type Plain RevokeReviewVerdictAcceptanceRequest
	var plain Plain
	if err := json.Unmarshal(value, &plain); err != nil {
		return err
	}
	*j = RevokeReviewVerdictAcceptanceRequest(plain)
	return nil
}
