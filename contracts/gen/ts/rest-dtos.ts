...
/**
 * GET /api/analytics response body (§12.2 item 6): the platform-wide analytics rollup ReviewAnalytics above deliberately does NOT cover (that endpoint is repo-scoped). windowDays names the rollup's own bounded window explicitly (platform.Timeouts.PlatformAnalyticsWindow) rather than a client-side hardcoded '30 days', mirroring RepoDigestScope.lookbackDays' own identical 'state the window honestly, from the server' precedent. sessionsTotal and falseFailureCount are a plain COUNT, a meaningful answer even at zero -- but that is only true of a count this deployment actually MANAGED to compute, so both carry their own computed sentinel too (sessionsTotalComputed/falseFailureCountComputed), false iff that rollup's own fetch failed, never false merely because the count itself is zero (internal/domain/platformanalytics's own doc comment explains the distinction). Every field on this DTO carries its OWN independent computed-or-not boolean, the SAME 'a repo with a real 0% and a repo with no data yet must never render identically' discipline ReviewAnalytics already establishes one level up -- see internal/app/platformanalytics for the full read model. Gated by the SAME authz.ActionViewAnalytics (§13.3 row 1) -- every role, including viewer.
 *
 * This interface was referenced by `RestDtos`'s JSON-Schema
 * via the `definition` "PlatformAnalytics".
 */
export interface PlatformAnalytics {
  /**
   * The rollup's own bounded window, in whole days (platform.Timeouts.PlatformAnalyticsWindow) -- every field below is scoped to sessions/turns/events created within the last windowDays days of the request.
   */
  windowDays: number;
  /**
   * False iff the session-outcome-counts rollup this count is drawn from could not be fetched -- sessionsTotal then stays 0, its own zero value, but that 0 is NOT a claim that zero sessions were created; it is 'unknown'. Never false merely because the real count itself is zero (a brand-new deployment's true 0 renders exactly the same as any other computed value). sessionsPerDayComputed/successRateComputed/topFailureReasonsComputed share this SAME underlying fetch and go false together with this field, since all four are reductions over the one same read (internal/app/platformanalytics.SessionOutcomeCountsInWindow's own doc comment).
   */
  sessionsTotalComputed: boolean;
  /**
   * Every session row created within the window, regardless of status -- INCLUDING a session created but never dispatched a single turn (session.StatusCreated). A meaningful count even at zero -- 0 is a true fact for a brand-new deployment, never 'unknown' -- PROVIDED sessionsTotalComputed is true; 0 while it is false means the count was never fetched, not that it is zero.
   */
  sessionsTotal: number;
  /**
   * False iff sessionsTotal's own window contains no session rows at all -- sessionsPerDay is then empty/null. Distinct from a real, computed set of buckets.
   */
  sessionsPerDayComputed: boolean;
  /**
   * One entry per UTC calendar day that had at least one session created within the window, oldest first. Null iff sessionsPerDayComputed is false.
   */
  sessionsPerDay: PlatformAnalyticsDayOutcomeBucket[] | null;
  /**
   * False iff no session in the window has DEFINITIVELY resolved (status completed or failed) -- a deployment with only Active/Created/Cancelled sessions has no success rate to report. successRatePercent is only meaningful when this is true.
   */
  successRateComputed: boolean;
  /**
   * completed / (completed + failed), as a 0-100 percentage. Deliberately excludes cancelled sessions from both the numerator and the denominator (a human decision, not a system judgment of success or failure) and excludes active/created sessions (no outcome yet). Null iff successRateComputed is false.
   */
  successRatePercent: number | null;
  /**
   * completed + failed -- the denominator behind successRatePercent, always populated (0 when successRateComputed is false) so a caller can render e.g. '92% of 20 resolved sessions' rather than a bare percentage.
   */
  successRateSampleSize: number;
  /**
   * False iff the false-failures COUNT query itself failed -- falseFailureCount then stays 0, its own zero value, but that 0 is NOT a claim that the target was met; it is 'unknown'. Never false merely because the real count itself is zero (a deployment that has genuinely never produced one renders exactly the same as any other computed value).
   */
  falseFailureCountComputed: boolean;
  /**
   * How many 'watchdog kill later proven alive' incidents (a session the control plane terminalized Failed/timeout that a late, genuine execution_complete then proved had actually succeeded) were detected within the window -- target 0. A meaningful count even at zero -- an incident either happened or it did not -- PROVIDED falseFailureCountComputed is true; 0 while it is false means the count was never fetched, not that the target was met.
   */
  falseFailureCount: number;
  /**
   * False iff no turn anywhere in the window ever recorded a cost figure (turns.cost_usd IS NULL for all of them) -- costTotalUsd/costMedianPerSessionUsd are only meaningful when this is true.
   */
  costComputed: boolean;
  /**
   * The straight sum of every costed turn's own cost_usd within the window. Null iff costComputed is false.
   */
  costTotalUsd: number | null;
  /**
   * The median of each SESSION's own total cost (summed across that session's turns) within the window -- the session is the unit, matching the tile's own 'median per session' label; a per-turn median would silently answer a different question. Null iff costComputed is false.
   */
  costMedianPerSessionUsd: number | null;
  /**
   * How many distinct sessions had at least one costed turn in the window -- the sample size behind both costTotalUsd and costMedianPerSessionUsd, always populated (0 when costComputed is false).
   */
  costSampleSize: number;
  /**
   * False iff costComputed is false (the same underlying 'has any cost data arrived' fact) -- costByModel is then empty/null.
   */
  costByModelComputed: boolean;
  /**
   * Every model this deployment dispatched a costed turn to within the window, spend descending. A turn whose own model_id was never recorded buckets under the literal string 'unknown' rather than being silently dropped, so these entries always sum to exactly costTotalUsd. Null iff costByModelComputed is false.
   */
  costByModel: PlatformAnalyticsModelCost[] | null;
  /**
   * False when bootP95SampleSize is below the minimum sample count a 95th-percentile estimate is considered trustworthy over (platformanalytics.BootP95MinSamples, 20) -- INCLUDING when it is exactly 0 (no boot_timing data at all). bootP95Seconds is only meaningful when this is true.
   */
  bootP95Computed: boolean;
  /**
   * The 95th percentile of every successful boot-to-ready duration recorded within the window (a failed boot attempt's own elapsed time is excluded -- it does not represent normal boot latency). Null iff bootP95Computed is false.
   */
  bootP95Seconds: number | null;
  /**
   * How many successful boot-duration samples were recorded within the window, always populated regardless of bootP95Computed -- lets a caller distinguish 'no data yet' (0) from 'too few samples so far' (nonzero but below the minimum), rather than collapsing both into one indistinguishable 'not available' state.
   */
  bootP95SampleSize: number;
  /**
   * False iff no session exists in the window at all. A non-null EMPTY topFailureReasons with this true is itself a real, computed answer (sessions existed, none failed or were cancelled) -- the boolean, never array emptiness alone, is what a client must branch on.
   */
  topFailureReasonsComputed: boolean;
  /**
   * Every session.FailureReason that occurred on a Failed or Cancelled session within the window, count descending then reason ascending. Restricted to the four typed reasons this deployment's own schema actually persists (cancelled/failed/timeout/never_started) -- never the more granular, free-text examples a design mockup might draw, which this deployment's own rows cannot honestly attribute a session to. Null iff topFailureReasonsComputed is false.
   */
  topFailureReasons: PlatformAnalyticsFailureReasonCount[] | null;
}
/**
 * One UTC calendar day's own per-status session counts -- PlatformAnalytics.sessionsPerDay's own per-day row (internal/domain/platformanalytics.DayBucket). All five session_status values are carried explicitly (unlike ReviewAnalyticsDayBucket's three Shippable counts) so this bucket's own counts always sum to exactly that day's own contribution to sessionsTotal -- a session still Active or merely Created is not invisible here.
 *
 * This interface was referenced by `RestDtos`'s JSON-Schema
 * via the `definition` "PlatformAnalyticsDayOutcomeBucket".
 */
export interface PlatformAnalyticsDayOutcomeBucket {
  /**
   * Midnight UTC of this bucket's own calendar day.
   */
  day: string;
...
/**
 * One model's own spend across the window -- PlatformAnalytics.costByModel's own per-model row.
 *
 * This interface was referenced by `RestDtos`'s JSON-Schema
 * via the `definition` "PlatformAnalyticsModelCost".
 */
export interface PlatformAnalyticsModelCost {
  /**
   * The dispatched model id (turns.model_id), or the literal 'unknown' for a turn that never recorded one.
   */
  modelId: string;
  totalUsd: number;
}
/**
 * One session.FailureReason's own occurrence count across the window's Failed/Cancelled sessions -- PlatformAnalytics.topFailureReasons' own per-reason row.
 *
 * This interface was referenced by `RestDtos`'s JSON-Schema
 * via the `definition` "PlatformAnalyticsFailureReasonCount".
 */
export interface PlatformAnalyticsFailureReasonCount {
  reason: 'cancelled' | 'failed' | 'timeout' | 'never_started';
  count: number;
}
/**
...
export interface Integration {
  ...
  /**
   * Whether every value this surface's own ingress adapter needs is present in platform.Config AND this deployment has actually enabled that surface (platform.Config.IngressEnabled, NARVI_INGRESS_ENABLED, §12.5's own ingress-optionality decision) -- NEVER the secrets themselves, not even shaped: no prefix, no length, no masked form, nothing about WHAT configures it. A surface this deployment never enabled reads false; a surface missing even one required value while enabled also reads false, never a half-working "connected but broken" state -- see internal/domain/integrations's own ConfiguredSlack/ConfiguredLinear/ConfiguredGitHub doc comments for the exact set each surface checks. A genuine two-valued field: platform.Load's own fail-fast boot gate refuses to start an enabled-but-incomplete surface, so a running deployment can only ever be observed in one of two states per surface -- enabled and fully configured (true), or not enabled here (false) -- never a third 'enabled but broken' state this field would need to distinguish. A screen must NOT present this as a live health check of anything.
   */
  configured: boolean;
  /**
...
export interface MCPAuthorization {
  ...
  createdAt: string;
  /**
   * When this authorization itself lapses, platform.Timeouts.MCPGrantMaxLifetime after the latest consent: the latest any install of the client can keep refreshing without consenting again. Each consent's own refresh chain keeps the end it began with, MCPGrantMaxLifetime after that consent, so an install connected by an earlier consent must consent again sooner (technical plan §43.16).
   */
  expiresAt: string;
  /**
   * When the client last called /mcp under this authorization, to within platform.Timeouts.MCPGrantLastUsedWriteInterval; null if it never has. goJSONSchema forces the literal *time.Time for the same named-pointer-type reason Plan.decidedAt documents in full.
   */
 ...
 */
export interface ListMCPAuthorizationsResponse {
...
export interface MCPClient {
  ...
  /**
   * Every registered redirect URI. The authorization endpoint compares a presented URI against these exactly, except that the port of a registered http://127.0.0.1 or http://[::1] URI is ignored (technical plan §43.15).
   */
  redirectUris: string[];
  /**
...
/**
 * GET /api/mcp-clients's own response body (technical plan §43.15): every registered MCP client, oldest first. Admin only (authz.ActionManageIntegrations).
 *
 * This interface was referenced by `RestDtos`'s JSON-Schema
 * via the `definition` "ListMCPClientsResponse".
 */
export interface ListMCPClientsResponse {
  clients: MCPClient[];
}
/**
 * POST /api/mcp-clients's own request body (technical plan §43.15): pre-register an MCP client. The server generates the client_id. Every redirect URI must be at most 2048 bytes of printable ASCII: https with any host, or http on 127.0.0.1, [::1] or localhost, with a valid port if it names one, no fragment and no userinfo, and its host written in plain ASCII -- no percent sign in the authority, so an internationalized host is written in its xn-- form -- anything else is refused 400 (internal/domain/mcpclient). clientName is trimmed and must be 1 to 100 printable characters -- letters, marks, numbers, punctuation, symbols and the ASCII space -- so a control or invisible formatting character, a space other than U+0020 (a no-break, ideographic or thin space; white space at either end is trimmed, not refused), a line or paragraph separator, and a private-use or unassigned code point are all refused 400, as is a name stacking more than three combining marks on one character (the rule every registration path shares, technical plan §43.15).
 *
 * This interface was referenced by `RestDtos`'s JSON-Schema
