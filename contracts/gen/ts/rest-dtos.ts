...
export interface Automation {
  ...
  /**
   * W7 audit fix (confirmed MEDIUM finding: "the column added to end a silent failure is itself unread"): set the moment a machine-originated GitHub (check_run/status) or Linear dispatch is denied because this automation's own creator is not a linked, non-disabled account holding authz.ActionCreateSession (migrations/000138_automations_creator_unauthorized.up.sql), preserving the FIRST denial's own timestamp across repeated denials; cleared back to null the moment a machine-origin dispatch for this same automation is authorized again. Null means never denied for this reason, or has since recovered. An automation can be 'active' in the status column above and still be structurally incapable of ever firing because of this -- the product's own state was a lie about that until this field existed on the wire.
   */
