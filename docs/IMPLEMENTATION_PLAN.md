...
| 171 | sandbox-agent must not run git against a runtime-owned `.git` | The root cause Step 130 recorded and could not fix. §30.5 gives the in-sandbox runtime ownership of the workspace, `.git` included, so it selects any command git will later execute on sandbox-agent's behalf — content filters, merge drivers, and the uploadpack/receivepack transport pair are three demonstrated instances, and the list is open because the mechanism, not the key, is the defect. No flag, attributes override, or file written into `.git` by a process that does not own `.git` can close it; Step 130 measured one such attempt at 0 of 60 racing trials blocked. The remedy is structural and needs a decision before code: run git as a uid distinct from the workspace owner, or against git metadata the runtime cannot write (an ephemeral `--git-dir` the runtime has no handle on), or stop granting the ownership §30.5 grants and give the runtime a narrower path to committing. Each trades differently against the reason §30.5 exists, which is why this is filed rather than assumed | §30.5 |
