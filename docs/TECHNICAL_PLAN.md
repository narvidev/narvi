...
refreshes go on, nor any other client; blocking the flooding networks, or cutting the client off, is the
operator's answer (`docs/runbooks/mcp-client-cutoff.md`).
...
with concurrent authorizations from as many networks and finds exactly the cap stored.
