...
open redirect on one host can never lend its name to a document served by another; no proxy from the
environment
...
nor is asked again on each one. A later failure neither restarts nor extends that grace: past it, the
