...
**A confirmed fast-forward on the base is tolerated, and that tolerance carries its own residual (further amendment, found reviewing Step 173's own implementation — corrected a second time after the first correction itself asserted a proof that does not hold, found reviewing this Step's fourth adversarial round; corrected a THIRD time after the second correction's own counter-example put the cancelling commit on the wrong branch and could not actually be reproduced, found by a later adversarial sweep for cross-file contradictions and fixed only after reproducing the true shape in a scratch git repository).** Refusing on ANY base-SHA movement — even an ordinary, unrelated merge landing on an otherwise-untouched trunk — would disqualify every verdict on an active repository the moment anyone else merges, with no mechanism here to win eligibility back except a fresh review nobody asked the PR's own author to seek: §24's automatic re-review watches the PR's own head, never its base. The eligibility engine tolerates exactly one shape of base movement instead of refusing outright: base ref unchanged, and the base's recorded commit confirmed — via a live ancestor check against the base branch's current tip — to be an ancestor of (or identical to) it, i.e. the base only ever gained new commits, never rewound or rewritten. **This does not bound the diff to a subset of what the verdict already examined.** Verified against real git, reproduced in a scratch repo: head, not base, carries both halves of the cancelling pair — head branches from a shared ancestor, applies C1 (the change) then, later, its own C2 (reverting C1), so head's tip already contains both. At verdict time, base has forked BEFORE C1 and never received it, so the merge-base is the shared pre-C1 ancestor, and the three-dot diff (merge-base...head) shows nothing for that file: C1 and C2 cancel against that merge-base. Base then merges in the branch that carries C1 (never the reverse — base is catching up to a change head already had, not reacting to something base itself did) — a confirmed fast-forward under this section's own tolerance. That merge alone advances the merge-base from the shared pre-C1 ancestor to C1 itself, and the fresh three-dot diff now contains a real hunk for that file: C2's own revert, previously invisible only because it canceled against C1 in the OLD diff — not a subset of nothing, a hunk the merge-base's own forward movement unmasks. Advancing the merge-base along head's own history does not monotonically shrink the diff: it can just as easily surface whatever an earlier commit on head changed and a later commit on head undid. What actually limits the exposure is narrower, and structural rather than diff-shaped: the engine's diff-size and sensitive-path criteria are always recomputed live from GitHub's current changed-files listing, never from the verdict's own stored diff, so a base movement that unmasks a change to a sensitive path, or that pushes the file count over threshold, is still caught. The residual that survives is narrower still: a base movement that silently unmasks a non-sensitive file's revert of a teammate's just-landed change, while the file count stays under threshold, is invisible to every check this engine runs — `Shippable == auto` stands, from a verdict that never saw the revert. It is **not** sound for the *combination* either: the verdict's `Shippable` and blast-radius judgement were formed by reading the diff against the base's OLD content, and `CIGreen` is checked at the PR's own head alone, a build that never re-runs against the base's new tip. Nothing here re-examines head's already-reviewed diff merged into what the base has since become — an accepted residual, structurally the same trade the stacked-PR residual two paragraphs above makes, but with trunk's own unrelated forward movement standing in for a sibling PR's, and closable, if it is ever closed, by one of two mechanisms neither built today: requiring CI to run against the merge result rather than the head alone, or leaning on a repository's own "base must be up to date" merge protection.
...
reported about its own work. §21.1's staleness rule applies, in the amended form that section
now states: head equality alone does not establish freshness, because a PR whose base moved
beneath it keeps its head and would pass an equality check while its verdict covers a context that
no longer exists. The gate is the verdict's whole persisted context — head, base, ancestor chain
and policy version — matching the PR as it stands now. Until Step 173 lands, this gate is as narrow
as the head-only check it inherits, and a train advancing on a retargeted predecessor is the shape
of the failure that narrowness permits.
...
exactly like a disabled client.
...
Row 181 ships in four pieces behind the existing flag (§43.11). (a) is the authorization-code core:
...
a revocation: its own, its user's, or reuse detection's.
...
document is Narvi's own small struct — issuer, the authorization, token and revocation endpoints, the
...
second, escaped; for a dynamically registered client: that it registered itself, so nothing vouches for
its name), the host the browser will be sent back to — with an explicit warning when it is this machine
...
exchange) is `invalid_target`. Success issues one access token (`MCPAccessTokenTTL`) and one refresh
token (`MCPRefreshTokenTTL`), neither past the grant's own expiry, both holding exactly the code's
scopes — the refresh token beginning a refresh chain bound to the code's resource and ending when the
grant ends as of this exchange (§43.16) — and answers `access_token`, `token_type` `Bearer`, `expires_in`, `scope` — always present,
...
`resource` that is sent must be this deployment's canonical resource, and so must the refresh chain's
own — the code's, fixed when the chain began, never the grant's, which a later consent rebinds in place
(`invalid_target` either way); a standard OAuth library sends none, since the chain already binds the
...
token or a refresh token, expired or rotated or not (an expired one until the expired-credential sweep
deletes it; after that it is unknown); the hint only decides which is looked up first — was issued to
...
URL. The URL must be printable ASCII — so an internationalized host is only ever seen in its `xn--` form
— with a host and a path other than `/`, and no userinfo, fragment or dot segment; it is the client's
identity and is compared byte for byte, never normalized. The authorization endpoint is the only place a
...
downgrade and never another scheme on a redirect; at most three redirects; no proxy from the environment
(a proxy would dial out of the guard's reach); `200` with `Content-Type: application/json` only; at most
64 KiB (one byte more is read, and refused); one timeout, `MCPClientMetadataFetchTimeout`, for the whole
fetch.
...
refuses the authorization with an error page, never a redirect. A stale document whose re-fetch fails —
the host down, or the new document invalid — is kept for one more `MCPClientMetadataCacheTTL` and
logged, so a document host that is down neither breaks every authorization nor is asked again on each
one. A disabled client's document is never fetched again. **A re-fetch changes what the next
...
bidirectional overrides, zero-width characters — or otherwise unprintable character, the rule
pre-registration applies too); `redirect_uris` present, non-empty, each registrable under the redirect
...
peer's address alone (`RemoteAddr`, an IPv6 address by its /64) and never a forwarded header, whose
trust is a deployment decision about the proxy chain not yet made. Over its budget the answer is `429`
with `Retry-After`. The limiter bounds its own memory and fails closed when full; it is built to be
reused for the token and authorization endpoints in piece (d).
...
registrations; like a disabled client, such a client may still give its tokens back (§43.14).
Pre-registered clients are unaffected by either flag.
...
client with no grant and no authorization request once it was registered — or, for a metadata-document
client, last fetched — more than `MCPDynamicClientUnusedTTL` (24 hours) ago. Pre-registered clients are
never swept, and a client whose consent is in flight always has its request, so it is never a
candidate.
...
a user holds at most one per client: consenting to the same client again — from a second install whose
`client_id` is shared, or after a refresh token lapsed — renews the grant's lifetime in place (same id)
rather than adding a row, so neither the table nor the Connected apps list grows with every consent.
**What a credential may do is fixed when it is issued:** an authorization code carries exactly the
scopes approved in the consent decision that issued it; the access token and the refresh token it is
exchanged for copy them; a refresh copies the PRESENTED refresh token's scopes into the tokens it issues,
or narrows them; and the bearer check reads the access token's own scopes. None of these ever reads the
grant's. The grant's `scopes` only record the most recent approval, for the Connected apps list, and no
authorization decision reads them, so a later consent for the same client can neither widen nor narrow
a credential already issued — not an access token, and not a refresh chain either. **Nor can it extend
or rebind a refresh chain:** a chain — the refresh tokens one code exchange begins and every rotation
continues — carries its own resource, copied from the code, and its own absolute end
(`chain_expires_at`), copied from the grant's expiry at that exchange, both carried unchanged through
every rotation. A refresh checks the chain's resource, never the grant's, refuses a chain past its end,
and caps every token it issues at the chain's end or the grant's own expiry, whichever comes first. So
re-consent renews the grant, and the chain its own code begins, and nothing issued earlier: a chain ends
`MCPGrantMaxLifetime` after the consent that began it, however often the user approves the client
again — which is what keeps a stolen chain from being carried past that limit by its victim's later
approvals. A later consent does not shorten an existing chain either: renewing the grant moves its
expiry later, past the chain's end (unless an operator lowered `MCPGrantMaxLifetime` in between, when
the grant's earlier end — at which the sweep deletes it — ends every chain under it). Withdrawing access
is revocation, not re-consent. The grant exists if and only if the
authorization is live: every authorization code, access token and refresh token references it with
`ON DELETE CASCADE`, so revocation is deleting the grant — nothing to invalidate, nothing to broadcast.
Codes, access tokens, refresh tokens and consent nonces are stored only as `platform.HashToken`
output; the plaintext code and tokens carry recognizable prefixes (`narvi_mcp_ac_`, `narvi_mcp_at_`,
`narvi_mcp_rt_`) so a secret scanner or a reviewer reading a log can tell the family, and the whole
string is hashed.

**Refresh tokens rotate on every use** (`mcp_oauth_refresh_tokens`, migration 000142): a successful
refresh marks the presented token rotated, names its successor, and issues a new refresh token — with a
fresh `MCPRefreshTokenTTL`, never past its chain's end or the grant's expiry — beside a new access token.
Access tokens issued earlier stay valid until their own expiry. A rotated token is kept until its own
expiry so that presenting it again, as any client, is recognised: that is a replay (OAuth 2.1 §4.3.1),
and the grant is deleted — the legitimate client's tokens with the thief's, since the server cannot tell
them apart — with an `mcp_authorization.revoked` audit row (reason `refresh_reuse`). A refresh token
presented as a client it was not issued to is a replay too, rotated or not: that client never received
it, so it holds a copy. There is **no grace window**: two refreshes racing with one token are one use
and one replay, whichever client sends the second — the second learns of the rotation from its own
...
extension of its cache after a failed re-fetch, are each one statement taking one row lock — the client,
...
deletion registers the document afresh. The unused-client sweep deletes exactly as an administrator's
deletion does, client first, and only a client with no grant and no request, so its cascade reaches no
row; an authorization racing it waits, then is refused with a page. A dynamic registration inserts a new
row and locks nothing that exists (`TestLockOrder_ClientRegistrationWriters`, both orders of every
pair).
...
granted, when it was last used, when it expires, and a Revoke action behind a confirmation — a person's
one way to withdraw access (the app itself may also give its tokens back, §43.14), since approving an
app again never takes back what an earlier approval issued, refresh chains included. `GET /api/me/mcp-authorizations`
...
| Client identity spoofing (confused deputy at registration) | the page shows who vouches for the client — an administrator; for a metadata document, its URL's host as the headline and the self-chosen name second, escaped; for a dynamic registration, nobody — and the true redirect host, with a loopback warning; a document's `client_id` must equal its URL byte for byte; a name with a control, bidi-override or other unprintable character is refused on every registration path; dynamic registration is off by default | `TestConsent_ShowsClientIdentityAndRedirectHost`, `TestCIMD_ClientIDMismatchRejected`, `TestCIMD_ClientIDMismatchRejectedAtAuthorization`, `TestCIMD_ValidationTable`, `TestValidateClientName`, `TestRegister_ValidationAndForcedFields`, `TestOAuth_ProductionRouter/Register_DisabledByDefault` |
| Metadata-document fetch as SSRF | one outbound adapter, constructible only with its guard; every address checked at dial time, after resolution, on every redirect and every re-resolution (loopback, private, link-local and the cloud metadata address, CGNAT, multicast, unspecified, reserved, and their IPv4-mapped and NAT64 forms); `https` only with no downgrade, at most three redirects, no environment proxy, `application/json` only, 64 KiB, one timeout; the only way past the guard is a test seam production never sets | `TestCIMDFetch_RefusesPrivateTargets`, `TestCIMDFetch_RefusesHTTPAndOversize`, `TestCIMDFetch_FetchesThroughTheSeamOnly`, `TestCheckAddr_Table`, `TestControl_ChecksTheConnectingAddress`, `TestOAuth_ProductionRouter/MetadataDocument_ProductionGuardRefusesLoopback` |
| A changed client document changing issued credentials | a re-fetched document changes what the next authorization sees, never an issued code's redirect binding, a token's scopes or a refresh chain; the cache is a fixed hour that cache headers only shorten; a stale document whose re-fetch fails is kept one more hour | `TestCIMD_RefetchNeverChangesIssuedCredentials`, `TestCIMD_CacheTTLOnlyShortened`, `TestCIMD_CacheTTLOnlyShortened_Stored`, `TestCIMD_AuthorizationEndpoint` |
| A registration mechanism switched off | every client it registered is refused like a disabled one — authorization, consent, token endpoint, `/mcp` — and may still revoke; a disabled metadata-document client's document is never fetched again | `TestClientMechanismSwitchedOff_RefusedEverywhere`, `TestMechanisms_Accepts`, `TestRequireMCPBearer_Table`, `TestOAuth_ProductionRouter/MetadataDocuments_Disabled` |
...
| Table growth | a TTL on every row kind, rotated refresh tokens included, swept; unused self-registered clients swept; dynamic registration braked per client address (`RemoteAddr` only), with bounded limiter memory | `TestExpiredCleanup_SweepsMCPRows`, `TestExpiredCleanup_SweepsUnusedMCPClients`, `TestRateLimiter_BurstThenOnePerInterval`, `TestRateLimiter_BoundedMemoryFailsClosed`, `TestClientAddressKey`, `TestOAuth_ProductionRouter/Register_RateLimited` |
...
packet reaches it; `EndToEnd_SDKClient_DynamicRegistration` has the SDK client register itself with the
flag on; and `Register_DisabledByDefault`, `Register_RateLimited` and `MetadataDocuments_Disabled` prove
the defaults, the brake and the kill switch.
