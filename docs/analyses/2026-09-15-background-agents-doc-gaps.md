...
Restent ouverts à la date de cette mise à jour : les cinq décisions de `docs/DECISIONS.md` elles-mêmes, A04 hors premier point (spécification des consommateurs, concurrence de publication, politique `not_assessed` côté GitHub, parcours d'acceptation — à écrire une fois le Step 173 livré, puisque tout s'appuie sur sa sémantique commune), et les deux recettes sans porteur ci-dessus.
